...
overall_change = cf.direct_comparison(df, col_variants=col_variant[0], unique_identifiers=uniq_id[0]
                                      , end_step=funnel[-1], start_step=funnel[0], start_step_unique=funnel_unq[0]
                                      , end_step_unique=funnel_unq[-1])
...
for i in range(len(funnel)-1):
    step_values = cf.direct_comparison(df, col_variants=col_variant[0], unique_identifiers=uniq_id[0]
                                       , end_step=funnel[i+1], start_step=funnel[i], start_step_unique=funnel_unq[i]
                                       , end_step_unique=funnel_unq[i+1])
...
print(cf.category_comparison(df=df, col_variants=col_variant[0], unique_identifiers=uniq_id[0]
                                       , end_step=funnel[-1], start_step=funnel[1], start_step_unique=funnel_unq[1]
                                       , end_step_unique=funnel_unq[-1], base_variant=1, category='cat3')[2])
